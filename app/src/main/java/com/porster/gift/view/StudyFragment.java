...
//        ((ContentLoadingProgressBar)$(R.id.study_load)).show();
        ...
        DataManager.getInstance().readListAsync(mContext, CACHE_HISTORY_STUDY, new DataManager.OnReadListener() {
            ...
            public void onSuccess(Object mObj) {
                ...
                ArrayList<GiftModel> list= (ArrayList<GiftModel>) mObj;
                ((ContentLoadingProgressBar)$(R.id.study_load)).hide();
                $(R.id.t_start).setEnabled(true);

                if(list!=null&&list.size()>0){
                    for (GiftModel giftModel : list) {
                        if(giftModel.selectAnswer.size()>4){
                            LogCat.w("ID="+giftModel.id+"的答案有5个");
                        }
                    }
                    ...
                    public void confirm() {
                        ApiUtils.execute(new AsyncTask<Object, Integer, Object>() {
                            ...
                            protected Object doInBackground(Object... params) {
                                DataManager.getInstance().saveList(mContext,CACHE_HISTORY_STUDY,new ArrayList<GiftModel>());
                                return null;
                            }
